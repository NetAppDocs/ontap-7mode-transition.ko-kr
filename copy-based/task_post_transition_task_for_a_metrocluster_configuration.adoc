...
.단계
. [[first-step]]스토리지 컷오버 시 생성된 cron 작업 메시지를 기록하여 원격 사이트에 복제해야 하는 작업 스케줄을 식별합니다.
+
image::../media/mcc_cron_jobs.gif[MCC cron 작업]

. 원격 사이트의 클러스터 CLI에서  `job schedule cron create`에 기록된 <<first-step,이전 단계>> 명령어를 실행합니다.
