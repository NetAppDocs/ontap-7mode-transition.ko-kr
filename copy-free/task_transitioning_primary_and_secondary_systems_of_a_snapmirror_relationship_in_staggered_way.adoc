...
.. [[_step_2b]]  `job schedule cron create` 명령을 사용하여 7-Mode SnapMirror 관계에 대해 구성된 일정과 일치하는 작업 일정을 생성하십시오.
.. [[_step_2c]]  `snapmirror create` 명령을 사용하여 7-Mode 운영 볼륨과 ONTAP 2차 볼륨 간에 TDP 유형의 SnapMirror 관계를 생성합니다.
...
..  `snapmirror delete` 명령을 사용하여 <<_step_2c,2c단계>>에서 생성된 7-Mode 운영 볼륨과 ONTAP 2차 볼륨 간의 TDP SnapMirror 관계를 삭제합니다.
.. <<_step_2b,2b단계>>에서 생성된 cron 작업 스케줄을 삭제합니다: +
